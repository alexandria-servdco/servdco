...
import { 
  Star, MapPin, Shield, Calendar, Clock, Users, ArrowRight, Heart, Check, Play, UserCircle, MessageSquare
} from "lucide-react";
...
const CHEFS_DATA: Record<string, any> = {
  sarah: {
    ...
    menus: [
      { course: "Main Course", title: "Herb Roasted Chicken", desc: "Served with local garlic mashed potatoes and seasonal roasted vegetables." },
      { course: "Main Course", title: "Grass-Fed Beef Pot Roast", desc: "Slow-braised with baby carrots, celery, and fresh garden rosemary." },
      { course: "Starter", title: "Farmhouse Salad", desc: "Crisp organic greens, heirloom cherry tomatoes, with house honey vinaigrette." }
    ],
    customerReviews: [
      { author: "Emily J.", date: "2 days ago", rating: 5, text: "Sarah was absolutely fantastic! The pot roast was cooked to perfection and she left our kitchen cleaner than it was before she arrived." },
      { author: "Marcus K.", date: "1 week ago", rating: 5, text: "We book Sarah for our weekly meal prep every single Sunday. Her dishes are incredibly fresh, and it has saved us so much time." }
    ]
  },
  michael: {
    ...
    menus: [
      { course: "Main Course", title: "Handmade Tagliatelle Ragu", desc: "Classic slow-braised beef and pork ragu with Parmigiano-Reggiano." },
      { course: "Main Course", title: "Wild Mushroom Risotto", desc: "Arborio rice cooked with rich porcini stock, fresh herbs, and white truffle oil." },
      { course: "Starter", title: "Caprese Bruschetta", desc: "Toasted sourdough, heirloom tomatoes, fresh mozzarella, and sweet basil." }
    ],
    customerReviews: [
      { author: "Antonio S.", date: "3 days ago", rating: 5, text: "The tagliatelle was out of this world! Michael is extremely professional and polite. Our kids loved watching him roll out the fresh pasta." }
    ]
  },
  priya: {
    name: "Chef Priya Patel",
    image: "https://images.unsplash.com/photo-1595273670150-db0d3c67adb8?w=600&auto=format&fit=crop&q=80",
    rating: "4.9",
    reviews: "186",
    location: "Dallas, TX",
    ...
    menus: [
      { course: "Main Course", title: "Creamy Paneer Tikka Masala", desc: "Spiced grilled paneer cubes in a luxurious, aromatic cashew tomato gravy." },
      { course: "Main Course", title: "Fragrant Dal Tadka", desc: "Slow-simmered yellow lentils tempered with cumin, garlic, and fresh ghee." },
      { course: "Bread / Starter", title: "Garlic Naan & Samosas", desc: "Warm flatbread baked fresh alongside crispy spiced potato pastries." }
    ],
    customerReviews: [
      { author: "Deepak R.", date: "4 days ago", rating: 5, text: "Incredibly authentic and flavorful. Priya customized the spice levels perfectly for our young children. Highest recommendation!" },
      { author: "Clara M.", date: "2 weeks ago", rating: 5, text: "Having Priya cook in our home has completely changed how we eat. Healthy, clean, and absolutely delicious Indian food." }
    ]
  }
};
...
export default function ChefProfile() {
  ...
  const chefId = id && CHEFS_DATA[id.toLowerCase()] ? id.toLowerCase() : "priya";
  ...
  const baseRate = serviceType === "breakfast" ? 40 : serviceType === "mealprep" ? 70 : 60;
    ...
    <div className="min-h-screen bg-[#111111] text-[#F5F5F5] font-sans selection:bg-[#FF7A59]/20 selection:text-[#FF7A59]">
      ...
      <section className="py-12 lg:py-20 bg-[#111111]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          {/* Back link */}
          <Link to="/browse-chefs" className="inline-flex items-center gap-2 text-xs font-bold text-[#A8A8A8] hover:text-[#FF7A59] mb-8 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-8 space-y-12">
                ...
                <div className="flex-1 space-y-4 text-center md:text-left">
                  <div className="space-y-1">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                      <h1 className="text-3xl font-bold text-white font-serif">{chef.name}</h1>
                      <button 
                      ...
                      >
                        <Heart size={13} className={favorite ? "fill-[#FF7A59]" : ""} />
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 text-xs text-[#A8A8A8] pt-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[#2E7D66] font-semibold">Trust & Safety Vetted</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                    {chef.specialties.map((s: string) => (
                      <span key={s} className="px-3 py-1 bg-white/5 rounded-full text-xs font-semibold text-white border border-white/5">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                <h2 className="text-xl font-bold font-serif text-white border-b border-white/5 pb-3">About the Chef</h2>
              ...
              <div className="space-y-6">
                <h2 className="text-xl font-bold font-serif text-white border-b border-white/5 pb-3">Sample Dining Menu</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {chef.menus.map((menu: any, index: number) => (
                    <div key={index} className="bg-[#161616] p-6 rounded-[24px] border border-white/5 shadow-lg space-y-2">
                      ...
                      <h3 className="font-bold text-white text-base pt-1">{menu.title}</h3>
                      <p className="text-xs text-[#A8A8A8] leading-relaxed font-medium">{menu.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Reviews */}
              <div className="space-y-6">
                <h2 className="text-xl font-bold font-serif text-white border-b border-white/5 pb-3">Customer Reviews</h2>
                <div className="space-y-4">
                  {chef.customerReviews.map((rev: any, index: number) => (
                    <div key={index} className="bg-[#161616] p-6 rounded-[24px] border border-white/5 shadow-lg space-y-3">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-[#A8A8A8]">{rev.date}</p>
                          </div>
                        </div>
              ...
              <div className="bg-[#2A2A2A] rounded-[32px] p-8 border border-white/5 shadow-2xl relative">
                ...
                {bookingBooked ? (
                  <div className="text-center py-10 space-y-4">
                    ...
                    <h3 className="text-2xl font-bold font-serif text-white">Session Requested!</h3>
                    <p className="text-xs text-[#A8A8A8] leading-relaxed max-w-[260px] mx-auto">
                      Your booking request has been sent to {chef.name}. You will be notified in your family dashboard once confirmed.
                    </p>
                    <div className="pt-4">
                      <Link
                        to="/dashboard"
                  ...
                  <form onSubmit={handleBookingSubmit} className="space-y-6">
                    <div>
                      <h3 className="text-xl font-bold font-serif text-white">Book Cooking Session</h3>
                      <p className="text-[#A8A8A8] text-xs mt-1">Select your dining parameters below.</p>
                    </div>

                    <div className="space-y-4">
                      {/* Date */}
                      <div>
                        <label className="block text-[10px] font-bold text-white uppercase tracking-wider mb-1.5">Select Date</label>
                        <input
                          type="date"
                          required
                          value={bookingDate}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-white uppercase tracking-wider mb-1.5">Service Type</label>
                        <select
                          value={serviceType}
                        ...
                        >
                          <option value="breakfast">Breakfast Cooking ($40)</option>
                          <option value="dinner">Dinner Dining ($60)</option>
                          <option value="mealprep">Weekly Meal Prep ($70)</option>
                        </select>
                      </div>

                      {/* Number of guests */}
                      <div>
                        <label className="block text-[10px] font-bold text-white uppercase tracking-wider mb-1.5">Number of Guests</label>
                        <div className="flex items-center justify-between bg-[#1A1A1A] border border-white/5 rounded-xl px-4 py-1.5">
                          ...
                          <span className="text-sm font-bold text-white">{guestsCount} guest{guestsCount > 1 ? 's' : ''}</span>
                          <button
                            type="button"
                    ...
                    <div className="pt-6 border-t border-white/5 space-y-3.5">
                      ...
                      {extraGuests > 0 && (
                        <div className="flex justify-between text-xs text-[#A8A8A8]">
                          ...
                          <span className="font-semibold text-white">+${guestFee}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-baseline pt-2 border-t border-white/5">
                        <span className="text-xs font-bold text-white uppercase tracking-wider">Total Est. Cost:</span>
                        <span className="text-3xl font-bold text-[#FF7A59] font-serif">${totalCost}</span>
                      </div>
                      <p className="text-[10px] text-[#A8A8A8] text-center leading-relaxed font-medium">
                        Groceries are bought separately based on ingredients you choose. Basic cleanup is fully included.
                      </p>
                    </div>
